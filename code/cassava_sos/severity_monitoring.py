...
def preProcessData(data):
    ...
    # add farmID
    def getFarmID(county, id):
        if "baringo" in county.lower():
            if id < 10:
                return f"BAR:0{id}"
            return f"BAR:{id}"
        elif "siaya" in county.lower():
            if id < 10:
                return f"SIA:0{id}"
            return f"SIA:{id}"
        elif "homabay" in county.lower():
            if id < 10:
                return f"HOM:0{id}"
            return f"HOM:{id}"
        elif "bungoma" in county.lower():
            if id < 10:
                return f"BUN:0{id}"
            return f"BUN:{id}"
        elif "busia" in county.lower():
            if id < 10:
                return f"BUS:0{id}"
            return f"BUS:{id}"
        elif "migori" in county.lower():
            if id < 10:
                return f"MIG:0{id}"
            return f"MIG:{id}"
        elif "kilifi" in county.lower():
            if id < 10:
                return f"KIL:0{id}"
            return f"KIL:{id}"
