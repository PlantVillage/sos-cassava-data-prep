...
def renameColumns(data):
    cols = ['experimental_data-plot_data-experimental_field-termite_number',
    ...
    for index, row in data.iterrows():
        for i in range(1,17):
            if i == 1:
                ...
                #cols = [col if "PARENT_KEY" not in col else "PARENT_KEY" for col in cols]
                #print(cols)
                temp_df = row[cols]
                replace_dict = {}
                for col in cols:
